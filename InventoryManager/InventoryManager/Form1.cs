...
namespace InventoryManager
{
    public partial class Form1 : Form
    {
        ...
        private bool ValidateUserInput()
        {
            ...
            if (quantity <= 0)
            {
                MessageBox.Show("Quantity must be positive", "Product Manager", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtQuantity.Focus();
            }

            if (price <= 0)
            {
                MessageBox.Show("Price must be above zero", "Product Manage", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtPrice.Focus();
                return false;
            }
